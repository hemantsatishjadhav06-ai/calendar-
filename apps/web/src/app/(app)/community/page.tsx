    ...
    <>
      ...
      <main className="content" id="main" style={{ maxWidth: 'none' }}>
        ...
        <div className={view === 'post' ? 'inbox' : ''}>
          {view === 'post' && <aside className="inbox-list" aria-label="Posts with comments">{(groups.data?.commentPostGroups ?? []).map((g: any) => <button key={`${g.channelId}:${g.externalPostId}`} className="inbox-item" aria-selected={selectedPost?.externalPostId === g.externalPostId} onClick={() => setSelectedPost({ channelId: g.channelId, externalPostId: g.externalPostId })}>{g.postTarget && <Avatar src={g.postTarget.channel.avatarUrl} name={g.postTarget.channel.displayName} network={g.postTarget.channel.network} size="sm" />}<div style={{ minWidth: 0, flex: 1 }}><div style={{ fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.postTarget?.text || `Post ${g.externalPostId.slice(-6)}`}</div><div className="subtle">{g.unanswered ? <b style={{ color: 'var(--fg)' }}>{g.unanswered} unanswered</b> : 'All answered'} · {g.count} total · {timeAgo(g.latestAt)}</div></div></button>)}{groups.data && !groups.data.commentPostGroups.length && <p className="subtle">No comments yet.</p>}</aside>}
...
function CommentCard({ c, focused, onFocus, ent, grid }: { c: any; focused: boolean; onFocus: () => void; ent: any; grid: boolean }) {
  ...
  return (
    <article id={`c-${c.id}`} className="comment" data-answered={answered} tabIndex={0} onFocus={onFocus} onClick={onFocus} style={focused ? { outline: '2px solid var(--focus)' } : undefined} aria-label={`${c.kind.toLowerCase()} from ${c.authorName ?? c.authorHandle ?? 'someone'} on ${NETWORK_LABEL[c.channel.network]}: ${c.text.slice(0, 60)}`}>
