  ...
  return (
    <main className="auth-page" id="main">
      <form className="auth-card" onSubmit={submit} aria-labelledby="login-title">
        <div className="row" style={{ marginBottom: 20 }}><span className="sidebar-logo">R</span><h1 id="login-title" style={{ margin: 0, fontSize: 22 }}>Sign in to Relay</h1></div>
        ...
        {needsTotp && <div className="field"><label htmlFor="totp">Authentication code</label><input id="totp" className="input" inputMode="numeric" autoComplete="one-time-code" autoFocus value={totp} onChange={e => setTotp(e.target.value)} /><span className="hint">From your authenticator app, or a recovery code.</span></div>}
