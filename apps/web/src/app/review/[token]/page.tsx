...
const NETWORK_LABEL: Record<string, string> = {
  ...
  BLUESKY: 'Bluesky', MASTODON: 'Mastodon', DEVTO: 'DEV.to', DISCORD: 'Discord',
};

async function getReview(token: string): Promise<any | null> {
  const base = process.env.API_URL || 'http://localhost:4000';
  try {
    const res = await fetch(`${base}/review/${encodeURIComponent(token)}`, { cache: 'no-store' });
...
const mediaList = (media: unknown): any[] => (Array.isArray(media) ? media : []);
    ...
    <main id="main" style={{ minHeight: '100dvh', background: 'var(--bg, #faf9f7)', padding: '32px 16px' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <header style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
          ...
          {data?.org && <span className="subtle" style={{ marginLeft: 'auto' }}>Review for {data.org}</span>}
        </header>

        {!data ? (
          <div className="card" style={{ textAlign: 'center', padding: '40px 24px' }}>
            <h1 style={{ fontSize: 18, margin: '0 0 6px' }}>Link not available</h1>
            <p className="subtle" style={{ margin: 0 }}>This review link is invalid or has expired. Ask the sender for a fresh link.</p>
          </div>
        ) : (
          <>
            <div style={{ marginBottom: 14 }}>
              <h1 style={{ fontSize: 18, margin: '0 0 2px' }}>Please review this post</h1>
              <p className="subtle" style={{ margin: 0 }}>{data.targets?.length ?? 0} channel{(data.targets?.length ?? 0) === 1 ? '' : 's'} — approve it or ask for changes below.</p>
            </div>
                  ...
                  <article key={t.id} className="card" style={{ padding: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <span className="subtle" style={{ fontSize: 13 }}>{NETWORK_LABEL[t.network] ?? t.network}{t.handle ? ` · @${t.handle}` : ''}</span>
                      </div>
                      {due && <span className="tag" style={{ marginLeft: 'auto' }}>{due.toLocaleString()}</span>}
                    </div>
            ...
            <ReviewActions token={token} existing={data.approval} />

            <p className="subtle" style={{ textAlign: 'center', marginTop: 22, fontSize: 12 }}>Shared via Cadence. Your decision is sent to the team; the post is published by them once approved.</p>
          </>
        )}
      </div>
    </main>
