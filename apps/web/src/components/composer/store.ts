...
interface ComposerState {
  ...
  setTagIds: (ids: string[]) => void; setMode: (m: ComposerState['mode'], dueAt?: string | null) => void; setRequestApproval: (b: boolean) => void; setAutoRepost: (m: ComposerState['autoRepost']) => void; setPanel: (p: ComposerState['panel']) => void; markAi: () => void;
...
export const useComposer = create<ComposerState>((set, get) => ({
  isOpen: false, baseText: '', baseMedia: [], linkPreview: null, targets: {}, activeTab: 'base', tagIds: [], mode: 'QUEUE', dueAt: null, requestApproval: false, aiAssisted: false, autoRepost: 'OFF', panel: null, dirty: false,
  open: o => {
    ...
    set({ isOpen: true, postId: o.postId, ideaId: o.prefill?.ideaId, templateId: o.prefill?.templateId, baseText: o.prefill?.text ?? (o.prefill?.url ? `\n${o.prefill.url}` : ''), baseMedia: o.prefill?.media ?? [], linkPreview: null, targets, activeTab: 'base', tagIds: [], mode: o.mode ?? (o.dueAt ? 'CUSTOM' : 'QUEUE'), dueAt: o.dueAt ?? null, requestApproval: false, aiAssisted: false, autoRepost: 'OFF', panel: null, dirty: !!o.prefill });
  },
  close: () => { if (get().dirty && !get().postId) saveDraft(get()); else clearDraft(); set({ isOpen: false }); },
  reset: () => { clearDraft(); set({ isOpen: false, postId: undefined, ideaId: undefined, baseText: '', baseMedia: [], linkPreview: null, targets: {}, activeTab: 'base', tagIds: [], mode: 'QUEUE', dueAt: null, requestApproval: false, aiAssisted: false, autoRepost: 'OFF', panel: null, dirty: false }); },
