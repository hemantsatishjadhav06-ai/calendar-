  ...
  return (
    <div className="grid" style={{ gridTemplateColumns: 'minmax(360px, 1fr) 420px', alignItems: 'start' }}>
      <div>
        ...
        {tab === 'settings' && <section className="card"><div className="field"><label htmlFor="nn">Nickname (internal)</label><input id="nn" className="input" value={data.nickname} onChange={e => set({ nickname: e.target.value })} /></div><div className="field"><label htmlFor="sp-addr">Address</label><input id="sp-addr" className="input" value={page.url} disabled /><span className="hint">Custom domains are coming; contact support to reserve one.</span></div><div className="field"><label htmlFor="seo">SEO description</label><input id="seo" className="input" value={data.header.description ?? ''} onChange={e => set({ header: { ...data.header, description: e.target.value } })} maxLength={160} /></div></section>}
