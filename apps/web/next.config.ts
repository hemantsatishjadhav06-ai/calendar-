...
const config: NextConfig = {
  ...
  async rewrites() {
    ...
    return ['graphql', 'auth', 'oauth', 'uploads', 'events', 'billing', 'ai', 'notify', 'r'].map(p => ({ source: `/api/${p}/:path*`, destination: `${API}/${p}/:path*` })).concat([{ source: '/api/graphql', destination: `${API}/graphql` }, { source: '/api/billing', destination: `${API}/billing` }, { source: '/api/events', destination: `${API}/events` }, { source: '/api/uploads', destination: `${API}/uploads` }]);
  },
