...
/**
 * The direct-to-storage video upload handshake, shared by every surface that
 * accepts a video: a post draft, an engagement reply, and a DM.
 *
 * The three differ only in what owns the upload and how it is authorized, both
 * of which stay in the controller. Everything here is the security-critical
 * part — key generation, prefix authorization, the size ceiling and the
 * container sniff — which must not drift between surfaces.
 */
trait StoresPresignedVideo
{
    /**
     * Generate a presigned upload URL for a video file.
     * The storage key is generated server-side — never trusted from the client.
     */
    private function signVideoUpload(string $workspaceId): JsonResponse
    {
        $key = 'tmp/media/'.$workspaceId.'/'.Str::uuid().'.mp4';
     ...
     * @param  array<string, mixed>  $logContext  Identifies the owner in rejection logs.
     */
    private function storeUploadedVideo(array $validated, string $workspaceId, array $logContext = []): JsonResponse
    {
        ...
        // tmp prefix entirely.
        $this->authorizeKey($key, $workspaceId);
        ...
        if ($size > Platform::maxVideoBytesCeiling()) {
            ...
            Log::warning('Rejected oversize video upload', [...$logContext, 'size' => $size]);
        ...
        if (! $this->looksLikeMp4($disk, $key)) {
            Storage::disk($disk)->delete($key);
            Log::warning('Rejected non-MP4 video upload', $logContext);
            abort(422, 'Uploaded file is not a valid MP4 video.');
        }

        $final = 'media/'.$workspaceId.'/'.Str::uuid().'.mp4';
